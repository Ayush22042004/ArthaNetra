...
const MPDetail = () => {
  ...
  const contractorSignals = works
    .map(work => ({
      contractor:
        work.contractor ||
        work.contractorName ||
        work.agency ||
        work.implementingAgency ||
        work.executingAgency ||
        'Agency not recorded',
      rating: Number(work.quality_rating || work.qualityRating || 0),
      amount: Number(work.finalAmount || work.sanctionedAmount || work.recommendedAmount || work.amount || 0),
    }))
    .filter(signal => signal.contractor !== 'Agency not recorded' || signal.rating > 0)

  const contractorRating =
    contractorSignals.length > 0
      ? contractorSignals.reduce((total, signal) => total + (signal.rating || 3), 0) /
        contractorSignals.length
      : Math.max(2.5, Math.min(5, ((completionRate || 0) / 100) * 5))

  const contractorSpend = contractorSignals.reduce((total, signal) => total + signal.amount, 0)
  ...
  return (
    <div className="mp-detail-page">
      <div className="mp-detail-header">
        ...
        <div className="mp-contractor-intel">
          <div className="contractor-score">
            <FiStar />
            <div>
              <span>Contractor performance</span>
              <strong>{contractorRating.toFixed(1)} / 5</strong>
              <em>
                {contractorSignals.length
                  ? `${contractorSignals.length} work records with agency/rating signal`
                  : 'Derived from completion and utilization until ratings sync from DB'}
              </em>
            </div>
          </div>
          <div className="contractor-release-note">
            <span>Milestone release model</span>
            <strong>
              {formatINRCompact(contractorSpend || mp.totalExpenditure || 0, {
                includeRupeeSymbol: true,
              })}
            </strong>
            <em>Future payments unlock only after evidence-backed milestones.</em>
          </div>
        </div>
      </div>
